...
架构：**静态前端 + 自动数据更新管线 + Anthropic-compatible 多模型网关**。前端是纯静态站（Vite + React + TypeScript + Tailwind CSS），只消费 `src/data/models.json` 与 registry 元数据，**不含任何 API Key**；数据由 GitHub Actions 每周抓取官方定价页自动更新（Zod 校验 + 变更报告），push 到 main 后自动部署 GitHub Pages；网关（`npm run gateway`）在本地/自有服务器运行，用同一份 Anthropic Messages 请求格式按 `model` 路由到各家的 Anthropic 兼容端点。
...
npm run gateway       # 启动多模型网关（默认 http://localhost:8788，key 读 .env）
```
...
| **多模型网关** | 从 registry 生成的可调用模型清单（对照表中带「网关可调」标记），配 curl 用法示例；网关本体在本地运行，见下文 |
...
│       ├── Presets.tsx          # 预设模式
│       ├── Scenarios.tsx        # 场景推荐
│       ├── GatewayModels.tsx    # 网关模型清单（registry 生成，仅展示元数据）
│       └── ui.tsx               # 通用小组件
└── README.md
```
...
curl localhost:8788/v1/messages \
  -H "content-type: application/json" \
  -d '{"model": "deepseek-v4-flash", "max_tokens": 256,
       "messages": [{"role": "user", "content": "你好"}]}'
```

任何 Anthropic SDK 也可直接指向网关：`new Anthropic({ baseURL: "http://localhost:8788", apiKey: "unused" })`（网关不校验入站 key，且会剥离入站鉴权头，只发自己构造的头）。
...
- 网关**不部署到 GitHub Pages**（Pages 只有静态文件）；线上 Dashboard 的「05 · Gateway」区块只展示 registry 元数据。
...
- CI 里 `ANTHROPIC_API_KEY`（及可选的其他 provider key）**只**存 GitHub repo Settings → Secrets and variables → Actions，仅 `update-data.yml` 的管线步骤可见；本地 key 存 `.env`（已 gitignore）。
- 管线与网关代码在 `pipeline/`、`gateway/`，不被 `src/` 引用，不进 Vite bundle；前端只 import 静态 `models.json` 与 `registry.json`（后者仅含环境变量**名**）。
- 网关剥离入站鉴权头、响应与错误信息永不包含 key 值（缺 key 时只提示变量名）。
- 部署 workflow 构建后 grep key **值**的形态（`sk-ant-…`/`sk-…` 长串），命中即中止部署（兜底自检；变量名合法出现在产物中，用于 registry 展示）。
- 本地跑管线：`ant auth login` 后 SDK 自动取凭证，或写入 `.env` / `export ANTHROPIC_API_KEY=...`。
